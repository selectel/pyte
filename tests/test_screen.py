...
def test_remove_non_existant_attribute():
    ...
    assert screen.to_list() == [[screen.default_char, screen.default_char]] * 2
...
def test_resize():
    ...
    assert len(screen.to_list()[0]) == 2
    assert screen.to_list() == [[screen.default_char, screen.default_char]] * 2

    screen.resize(3, 3)
    assert screen.columns == screen.lines == 3
    assert len(screen.to_list()) == 3
    assert len(screen.to_list()[0]) == 3
    assert screen.to_list() == [
...
def test_draw_width2_irm():
    ...
    assert screen.to_list() == [[Char("コ"), Char(" ")]]
...
def test_erase_in_line():
    ...
    assert screen.display == ["     ",
                              ...
                              "re yo",
                              "u?   "]
    assert screen.to_list()[0] == [screen.default_char] * 5
...
def test_erase_in_display():
    ...
    assert screen.display == ["     ",
                              ...
                              "     ",
                              "     "]
    assert screen.to_list() == [[screen.default_char] * 5] * 5
